...
public class frmClearHistory : Form
{
	...
	private void RemoveHistory(DateTime endDate)
	{
		...
		do
		{
			num = Convert.ToInt32(DataBaseUtilities.DBOperation(Program.ConnectionString, $"select count(*) from ResumeHistory where PackDate <= #{arg}#", null, CommandOperationType.ExecuteScalar));
			if (num > 0)
			{
				ssLblDesc.Text = $"目前尚有{num}筆資料準備刪除，請稍候";
			}
			else
			{
				ssLblDesc.Text = "刪除成功!";
			}
			DataBaseUtilities.DBOperation(Program.ConnectionString, $"delete * from ResumeHistory where initPrintCodeSrt IN ( SELECT TOP 500 initPrintCodeSrt FROM ResumeHistory WHERE PackDate <= #{arg}# ORDER BY PackDate ASC ) ", null, CommandOperationType.ExecuteNonQuery);
